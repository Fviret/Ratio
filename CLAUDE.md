...
- **UI** : `src/app/decisions/page.tsx` (Server Component) charge la liste complète et rend `DecisionsList` (`src/app/decisions/decisions-list.tsx`, Client Component) — même split Server/Client que `new/`. `DecisionsList` gère la barre de recherche (form + bouton), appelle `POST /api/search`, affiche les résultats avec badge de pertinence normalisé (1er résultat = 100 %, les suivants en proportion). En dehors du mode recherche, la liste pré-chargée côté serveur est affichée sans appel réseau.
- La page détail (`[id]/page.tsx`) affiche tous les champs : context, options, decision_text, rationale, decider, stakeholders, decided_at, source_raw.
